...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride_conv, padding_size)

        self.conv.weight.data.mul_(weight_scale)

        conv_height = int((height + 2*padding_size - kernel_size)/stride_conv) + 1
        conv_width = int((width + 2*padding_size - kernel_size)/stride_conv) + 1

        effective_height = int(conv_height/2)
        effective_width = int(conv_width/2)

        self.fc1 = nn.Linear(int(effective_height*effective_width*num_filters), hidden_dim)
    ...
    def forward(self, x):
        ...
        x = self.max_pool(F.relu(self.conv(x)))
